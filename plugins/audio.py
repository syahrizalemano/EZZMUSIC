...
async def play(client, m: Message):
    msg = await m.reply_text("🔄 `Proses ktl, tunggu sebentar ...`")
    chat_id = m.chat.id
    ...
    if not media and not ' ' in m.text:
        ...
        if match:
            await msg.edit("🔄 `Youtube audio streaming dimulai ...`")
            try:
            ...
            await msg.edit("🔄 `Live audio streaming dimulai ...`")
            link = query
        ...
        try:
            ...
            AUDIO_CALL[chat_id] = group_call
            await msg.delete()
            await m.reply_text(f"▶️ **Started [Audio Streaming](https://t.me/ezzraez) In {m.chat.title} !**",
               reply_markup=InlineKeyboardMarkup(
               [
                   [
                       InlineKeyboardButton(
